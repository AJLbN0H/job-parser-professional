...
2. Install dependencies (the **dev** group includes Pytest):
...
3. Run the console app from the project root:
...
If you prefer a venv and pip, install runtime and test tools to match `pyproject.toml`:
...
pip install "requests>=2.32.3,<3" pytest pytest-cov
pytest
```

Run the app (project root on `PYTHONPATH`):
